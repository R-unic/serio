...
export type RestType<T extends readonly unknown[]> = T[9e99];
export type HasRest<T extends readonly unknown[]> = RestType<T> extends undefined ? false : true;

// This is a somewhat expensive type, so we only rebuild the tuple if we know `T` has a rest element.
export type SplitRest<T extends readonly unknown[], Acc extends unknown[] = []> =
